...
void main(void)
{
  ...
  TI_USCI_I2C_transmitinit(0x50,0x12);// init transmitting with USCI and DMA
  while ( TI_USCI_I2C_notready() );         // wait for bus to be free
  if ( TI_USCI_I2C_slave_present(0x50) )    // slave address may differ from
  {                                         // initialization
    TI_USCI_I2C_receiveinit(0x50,0x12);  // init receiving with USCI and DMA
    while ( TI_USCI_I2C_notready() );         // wait for bus to be free
    ...
    TI_USCI_I2C_transmitinit(0x50,0x12);// init transmitting with USCI and DMA
    while ( TI_USCI_I2C_notready() );         // wait for bus to be free
    TI_USCI_I2C_transmit(4,array);       // start transmitting with DMA
  }
